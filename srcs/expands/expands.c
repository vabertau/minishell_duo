...
/*   Updated: 2024/05/07 15:01:54 by vabertau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    replace_var(t_data *data, char **word, int i, int len_var)
{
	...
	ft_strlcat(ret, &((*word)[i + len_var + 1]), ft_strlen(ret) + ft_strlen(&((*word)[i + len_var + 1])) + 1); //error to suppress
    //printf("ret2 = %s\n", ret);
	free(*word);
    *word = ret;
...
int expands(t_data *data)
{
    ...
    {
        if (!(data->is_bq[i] == 1) && ft_strchr(tmp->word, '$')) // if it is to expand
